...
async function executeAdminPayout(adminKeypair: Keypair, payeePublicKey: PublicKey, lamports: number) {
  ...
  try {
    const txid = await solanaSendAndConfirmTransaction(connection, transaction, []);
